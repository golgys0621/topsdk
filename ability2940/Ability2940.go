...
/*
    通过时间段批量查询零售单据信息
*/
func (ability *Ability2940) AlibabaAlihealthDrugYljgRetailSearchbill(req *request.AlibabaAlihealthDrugYljgRetailSearchbillRequest) (*response.AlibabaAlihealthDrugYljgRetailSearchbillResponse,error) {
    if(ability.Client == nil) {
        return nil,errors.New("Ability2940 topClient is nil")
    }
    var jsonStr,err = ability.Client.Execute("alibaba.alihealth.drug.yljg.retail.searchbill",req.ToMap(),req.ToFileMap())
    var respStruct = response.AlibabaAlihealthDrugYljgRetailSearchbillResponse{}
    if(err != nil){
        log.Println("alibabaAlihealthDrugYljgRetailSearchbill error",err)
...
/*
    查询单据详情
*/
func (ability *Ability2940) AlibabaAlihealthDrugYljgSearchbillDetail(req *request.AlibabaAlihealthDrugYljgSearchbillDetailRequest) (*response.AlibabaAlihealthDrugYljgSearchbillDetailResponse,error) {
    if(ability.Client == nil) {
        return nil,errors.New("Ability2940 topClient is nil")
    }
    var jsonStr,err = ability.Client.Execute("alibaba.alihealth.drug.yljg.searchbill.detail",req.ToMap(),req.ToFileMap())
    var respStruct = response.AlibabaAlihealthDrugYljgSearchbillDetailResponse{}
    if(err != nil){
        log.Println("alibabaAlihealthDrugYljgSearchbillDetail error",err)
        return &respStruct,err
    }
    err = util.HandleJsonResponse(jsonStr, &respStruct)
    if(respStruct.Body == "" || len(respStruct.Body) == 0) {
