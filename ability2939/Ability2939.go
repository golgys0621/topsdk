...
/*
    通过时间段批量查询零售单据信息
*/
func (ability *Ability2939) AlibabaAlihealthDrugLsydRetailSearchbill(req *request.AlibabaAlihealthDrugLsydRetailSearchbillRequest) (*response.AlibabaAlihealthDrugLsydRetailSearchbillResponse,error) {
    if(ability.Client == nil) {
        return nil,errors.New("Ability2939 topClient is nil")
    }
    var jsonStr,err = ability.Client.Execute("alibaba.alihealth.drug.lsyd.retail.searchbill",req.ToMap(),req.ToFileMap())
    var respStruct = response.AlibabaAlihealthDrugLsydRetailSearchbillResponse{}
    if(err != nil){
        log.Println("alibabaAlihealthDrugLsydRetailSearchbill error",err)
...
/*
    查询单据详情
*/
func (ability *Ability2939) AlibabaAlihealthDrugLsydSearchbillDetail(req *request.AlibabaAlihealthDrugLsydSearchbillDetailRequest) (*response.AlibabaAlihealthDrugLsydSearchbillDetailResponse,error) {
    if(ability.Client == nil) {
        return nil,errors.New("Ability2939 topClient is nil")
    }
    var jsonStr,err = ability.Client.Execute("alibaba.alihealth.drug.lsyd.searchbill.detail",req.ToMap(),req.ToFileMap())
    var respStruct = response.AlibabaAlihealthDrugLsydSearchbillDetailResponse{}
    if(err != nil){
        log.Println("alibabaAlihealthDrugLsydSearchbillDetail error",err)
        return &respStruct,err
    }
    err = util.HandleJsonResponse(jsonStr, &respStruct)
    if(respStruct.Body == "" || len(respStruct.Body) == 0) {
